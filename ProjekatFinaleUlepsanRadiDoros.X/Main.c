...
int main(int argc, char** argv) {
	...
	while(1)
	{ 
        for(broj1 = 0; broj1<4; broj1++)
        {
            WriteUART1(rec[broj1]);
           // rec[broj1]=NULL;
        }
        if(rec[0] == 'D' && rec[1] == 'O' && rec[2] == 'R' && rec[3] == 'O' && rec[4] == 'S')
        {
            for(broj1=0; broj1<6; broj1++)
                rec[broj1] = NULL; // rec[4]='A' zato sto inace non stop menja stanja, te je taster = 1 te je taster = 0;
